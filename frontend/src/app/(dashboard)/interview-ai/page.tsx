...
export default function AIMockInterview() {
  ...
  const playTTS = (text: string) => {
    ...
    setTimeout(() => {
      ...
      // Try to find a specific voice for the target language to sound natural
  ...
  }, [step, currentQIndex, questions, language]);
  ...
  const startSetup = async () => {
    ...
    try {
      const res = await call("/api/ai/mock-interview/generate", {
        method: "POST",
        body: JSON.stringify({ role, techStack, experience, language })
      });
    ...
    try {
      ...
      recognition.onerror = (event: any) => {
         if (event.error === 'no-speech') {
             // Ignore quiet breathing, don't break the log
             return;
         }
  ...
  const processAudio = async (recordedTranscript: string) => {
    ...
    try {
      ...
      call("/api/ai/mock-interview/evaluate", {
        method: "POST",
        body: JSON.stringify({ question: currentQuestion, userAnswer: recordedTranscript, language })
      }).then(evalRes => {
        setEvaluations(prev => {
          const newArr = [...prev];
          newArr[idx] = {
    ...
    return (
      <div className="max-w-2xl mx-auto py-12 space-y-8 animate-in fade-in duration-500">
        <div className="space-y-2 text-center">
          ...
          <h1 className="text-3xl font-black tracking-tight">{t("interview.setup")}</h1>
          <p className="text-text-secondary text-sm">{t("interview.subtitle")}</p>
        </div>

        <div className="card-glass p-8 space-y-6 border-border-subtle">
          <div className="space-y-4">
            <div>
              <label className="text-xs font-bold uppercase tracking-widest text-text-muted mb-2 block">{t("interview.role")}</label>
              <input
                value={role} onChange={e => setRole(e.target.value)}
                className="w-full bg-hover-bg border border-border-subtle rounded-xl p-3 text-sm text-text-primary focus:outline-none focus:border-accent-start/50 transition-colors"
                placeholder="e.g. Backend Developer"
              />
            </div>
            <div>
              <label className="text-xs font-bold uppercase tracking-widest text-text-muted mb-2 block">{t("interview.techStack")}</label>
              <input
            ...
            <div>
              <label className="text-xs font-bold uppercase tracking-widest text-text-muted mb-2 block flex justify-between">
                {t("interview.experience")} <span className="text-accent-start">{experience} YOE</span>
              </label>
              <input
          ...
          >
            {isLoadingSetup ? <><Loader2 size={18} className="animate-spin" /> {t("interview.generating")}</> : t("interview.startInterview")}
          </button>
        </div>
      </div>
    );
        ...
        <div className="card-glass p-12 text-center space-y-6">
          ...
          <div>
            <h1 className="text-4xl font-black">
              {terminationReason ? (
              <>Interview <span className="text-red-400">{t("interview.sessionTerminated")}</span></>
            ) : (
            ...
            {terminationReason ? (
              <p className="text-red-400 font-bold mt-4 text-lg">{t("interview.terminationReason")}: {terminationReason}</p>
            ) : (
              <p className="text-text-secondary mt-2">{t("interview.results")}: <strong className="text-xl text-text-primary ml-2">{avgScore.toFixed(1)}/10 ({percentage}%)</strong></p>
            )}
          </div>
        </div>
              ...
              <div key={idx} className="card-glass p-8 space-y-6 border-border-subtle">
                ...
                <div className="flex justify-between items-start gap-4">
                  <div>
                    ...
                    <p className="text-lg font-bold text-text-primary">{evalData.question}</p>
                  </div>
  ...
  return (
    <div className="h-[90vh] py-4 flex flex-col space-y-4 max-w-7xl mx-auto animate-in slide-in-from-bottom-8 duration-500">
      ...
      <div className="grid lg:grid-cols-4 gap-6 flex-1 min-h-0">
        ...
        <div className="lg:col-span-3 flex flex-col gap-4 relative min-h-0 overflow-hidden h-full">

          {/* Question Prompt */}
          <div className="card-glass p-6 md:p-8 border-border-subtle shrink-0">
                  <span className="text-xs font-black uppercase tracking-[3px] text-accent-start mb-2 md:mb-4 block">{t("interview.question")} {currentQIndex + 1} {t("interview.of")} {questions.length}</span>
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold leading-tight text-text-primary line-clamp-3">
        ...
        <div className="lg:col-span-1 flex flex-col gap-4 min-h-0 overflow-hidden h-full">

          {/* Live Transcript (No feedback shown here!) */}
          <div className="card-glass border-border-subtle flex-1 flex flex-col p-6 min-h-0 overflow-hidden">
            <span className="text-[10px] font-black uppercase tracking-[2px] text-text-muted mb-4 block shrink-0">Candidate Transcript</span>
          ...
          <div className="card-glass p-6 border-border-subtle flex flex-col gap-4 shrink-0">
            <button
              ...
              className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 shadow-xl cursor-pointer ${isRecording ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-accent-start text-white hover:bg-accent-end'
                }`}
            >
